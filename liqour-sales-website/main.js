...
let countOfCart = 0;
cart.forEach((item) => {
  countOfCart += Number(item.quantity); 
});


const cartContainer = document.querySelector(".cart-container"); 
if (cartContainer) {
  const existing = cartContainer.querySelector(".cartLengthDisplay");
  if (existing) existing.remove();

  if (countOfCart) {
    const cartLengthDisplay = document.createElement("span");
    cartLengthDisplay.classList.add("cartLengthDisplay");
    cartLengthDisplay.innerText = countOfCart;
    cartContainer.appendChild(cartLengthDisplay);
...
profileContainer.addEventListener("mouseenter", () => {
  profileExpand.classList.add("profile-expand-active");
});
