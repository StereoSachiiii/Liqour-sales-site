...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    $sql = "SELECT * FROM warehouse 
            WHERE is_active=1 AND (name LIKE '%$q%' OR address LIKE '%$q%')
...
<div class="container">
    ...
    <div class="table-container">
        <table class="table" id="warehouse-table">
            ...
            <tbody>
                <?php
                $res = $conn->query("SELECT * FROM warehouse WHERE is_active=1 ORDER BY warehouse_id DESC LIMIT $records_per_page");
                if ($res && $res->num_rows > 0) {
                    while ($row = $res->fetch_assoc()) {
                        $id = htmlspecialchars($row['warehouse_id']);
                        $name = htmlspecialchars($row['name']);
    ...
    $('#search-input').on('input', function(e){
        ...
        ajaxTimeout = setTimeout(() => {
            ...
            $.get('search.php', { ajax: 1, q: query }, function(data){
                $('#warehouse-table tbody').html(data);
            });
        }, 300);
    });
