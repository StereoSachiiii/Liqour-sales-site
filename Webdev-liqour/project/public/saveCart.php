...
try {
    ...
    // 0. HANDLE ADDRESS - Process before creating order
    $finalAddressId = null;
    ...
    if ($newAddress) {
        ...
        if ($stmt->execute()) {
            $finalAddressId = $conn->insert_id;
            error_log("Created new address with ID: $finalAddressId");
        } else {
            $conn->rollback();
            echo json_encode(['status'=>'error','message'=>'Failed to save address: ' . $stmt->error]);
            exit();
        }
        $stmt->close();
    } elseif ($addressId) {
        // Verify the address belongs to this user
        $stmt = $conn->prepare("SELECT address_id FROM user_addresses WHERE address_id = ? AND user_id = ? AND is_active = 1");
        ...
        if ($result->num_rows > 0) {
            ...
            error_log("Using existing address ID: $finalAddressId");
        } else {
            $conn->rollback();
    ...
    $stmt = $conn->prepare("
        INSERT INTO orders (user_id, address_id, status, total, created_at, updated_at) 
        VALUES (?, ?, 'pending', ?, NOW(), NOW())
    ");
    $stmt->bind_param("iid", $userId, $finalAddressId, $total);
    if (!$stmt->execute()) {
        $conn->rollback();
        echo json_encode([
            "status" => "error", 
    ...
    error_log("Created order with ID: $orderId and address ID: $finalAddressId");
