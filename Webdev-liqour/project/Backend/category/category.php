...
if (isset($_GET['ajax']) && $_GET['ajax']==1) {
    ...
    if($res && $res->num_rows>0){
        ...
        $response['html'] = "<tr><td colspan='4' style='text-align:center; padding:2rem; color:#666;'>No categories found</td></tr>";
    }
...
$count_stmt = $conn->prepare("SELECT COUNT(*) as total FROM liqour_categories WHERE is_active=1");
$count_stmt->execute();
$total_records = $count_stmt->get_result()->fetch_assoc()['total'];
$total_pages = ceil($total_records/$records_per_page);
...
<title>Liquor Categories Management</title>
...
<h1>Liquor Categories Management</h1>
...
<input type="text" id="search-input" class="search-box" placeholder="Search by category name..." autocomplete="off">
...
if($res && $res->num_rows>0){
    ...
    echo "<tr><td colspan='4' style='text-align:center;padding:2rem;color:#666;'>No categories found</td></tr>";
}
?>
</tbody>
</table>
...
$(document).ready(function(){
    ...
    function loadCategories(query,page){
        $('.search-container').addClass('loading');
        $.ajax({
            url:window.location.pathname, // Use current file URL
            method:'GET',
            data:{ajax:1,q:query,page:page},
            dataType:'json',
            success:function(resp){
                $('#categories-table-body').html(resp.html).addClass('fade-in');
                $('#pagination-container').html(resp.pagination);
                setTimeout(()=>{$('#categories-table-body').removeClass('fade-in');},300);
            },
    ...
    $('#search-input').on('input',function(){
        ...
        ajaxTimeout=setTimeout(()=>{loadCategories(currentQuery,1);},300);
    });

    $(document).on('click','.page-btn',function(e){
        e.preventDefault();
        let page=$(this).data('page');
        if(page){currentPage=page;loadCategories(currentQuery,page);}
    });
});
</script>
