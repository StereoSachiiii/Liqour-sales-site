...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // MODIFIED: Added LEFT JOIN with suppliers table
    $sql = "SELECT l.*, c.name AS category_name, s.name AS supplier_name
            FROM liqours l
            JOIN liqour_categories c ON l.category_id = c.liqour_category_id
            LEFT JOIN suppliers s ON l.supplier_id = s.supplier_id
            WHERE l.is_active=1 AND (l.name LIKE '%$q%' OR c.name LIKE '%$q%' OR s.name LIKE '%$q%')
    ...
    if ($res && $res->num_rows > 0) {
        while ($row = $res->fetch_assoc()) {
            ...
            // MODIFIED: Added supplier column
            echo "<tr>
                    <td>{$id}</td>
                    <td>{$name}</td>
                    <td>{$category}</td>
                    ...
                    <td>
                        ...
                        <a href='delete.php?id={$id}&type=soft' onclick=\"return confirm('Soft delete this liqour?');\" class='btn soft-delete'>Soft Delete</a>
                        <a href='delete.php?id={$id}&type=hard' onclick=\"return confirm('Permanently delete this liqour?');\" class='btn delete'>Delete Forever</a>
                    </td>
                  </tr>";
        }
    } else {
        echo "<tr><td colspan='7'>No liqours found</td></tr>";
    }
    exit;
}
?>
...
<div class="container">
   ...
   Back to Dashboard
    ...
    <div class="section-header">
        <h2>Liquors</h2>
        <input type="text" id="search-input" class="search-box" placeholder="Search liquors, categories, or suppliers...">
    </div>

    <div class="table-container">
        <table class="table" id="liqours-table">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Name</th>
            ...
            <tbody>
                <?php
                // MODIFIED: Added LEFT JOIN with suppliers table
                $res = $conn->query("SELECT l.*, c.name AS category_name, s.name AS supplier_name
                                     FROM liqours l 
                                     JOIN liqour_categories c ON l.category_id = c.liqour_category_id
                                     LEFT JOIN suppliers s ON l.supplier_id = s.supplier_id
                                     WHERE l.is_active=1 
