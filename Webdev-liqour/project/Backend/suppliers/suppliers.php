...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    $count_stmt = $conn->prepare("SELECT COUNT(*) as total FROM suppliers WHERE is_active=1 AND (name LIKE ? OR email LIKE ?)");
    ...
    // Fetch suppliers
    $stmt = $conn->prepare("SELECT * FROM suppliers WHERE is_active=1 AND (name LIKE ? OR email LIKE ?) ORDER BY supplier_id DESC LIMIT ? OFFSET ?");
    ...
    <td>
        <div class='action-buttons'>
            <a href='update.php?supplier_id={$id}' class='btn'>Update</a>
            <a href='delete.php?supplier_id={$id}&type=soft' onclick=\"return confirm('Soft delete this supplier?');\" class='btn soft-delete'>Soft Delete</a>
            <a href='delete.php?supplier_id={$id}&type=hard' onclick=\"return confirm('⚠️ PERMANENTLY DELETE?');\" class='btn delete'>Delete Forever</a>
        </div>
    </td>
</tr>";
        }
    } else {
        $response['html'] = "<tr><td colspan='6' style='text-align:center; padding:2rem; color:#666;'>No suppliers found</td></tr>";
    }
...
$count_stmt = $conn->prepare("SELECT COUNT(*) as total FROM suppliers WHERE is_active=1");
$count_stmt->execute();
$total_records = $count_stmt->get_result()->fetch_assoc()['total'];
$total_pages = ceil($total_records/$records_per_page);

$stmt = $conn->prepare("SELECT * FROM suppliers WHERE is_active=1 ORDER BY supplier_id DESC LIMIT ? OFFSET ?");
...
.btn.soft-delete{background:var(--warning);color:#000;}
.btn.delete{background:var(--danger);color:#000;}
...
.btn{color:#000;padding:0.3rem 0.5rem;border-radius:var(--radius);font-size:0.8rem;font-weight:600;transition:var(--transition);cursor:pointer;text-decoration:none;color:#000;background:var(--primary);}
...
<nav class="nav">
    ...
    <a href="add.php" class="btn">Add New Supplier</a>
</nav>
</header>

<main class="main">
<section class="section">
<div class="section-header"><h2>Active Suppliers</h2></div>
<div class="section-content">
<div class="search-container">
    <input type="text" id="search-input" class="search-box" placeholder="Search by name or email..." autocomplete="off">
...
<table class="table">
<thead>
<tr><th>ID</th><th>Name</th><th>Email</th><th>Status</th><th>Created At</th><th>Actions</th></tr>
</thead>
...
if($res && $res->num_rows>0){
    ...
    echo "<tr><td colspan='6' style='text-align:center;padding:2rem;color:#666;'>No suppliers found</td></tr>";
}
?>
</tbody>
</table>
...
$(document).ready(function(){
    ...
    function loadSuppliers(query,page){
        ...
        $.ajax({
            ...
            success:function(resp){
                $('#supplier-table-body').html(resp.html).addClass('fade-in');
                $('#pagination-container').html(resp.pagination);
                setTimeout(()=>{$('#supplier-table-body').removeClass('fade-in');},300);
            },
    ...
    $('#search-input').on('input',function(){
        ...
        ajaxTimeout=setTimeout(()=>{ loadSuppliers(currentQuery,1); },300);
    });

    $(document).on('click','.page-btn',function(e){
        e.preventDefault();
        let page=$(this).data('page');
        if(page){ currentPage=page; loadSuppliers(currentQuery,page); }
    });
});
</script>
</body>
